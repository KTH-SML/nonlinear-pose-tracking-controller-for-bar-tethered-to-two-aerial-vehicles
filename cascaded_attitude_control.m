...
Module[{
	...
	\[Sigma]\[Sigma]=\[Sigma]d/.AttitudeControllerGains,
	...
	\[Tau]ff=OP[n].\[Tau]d+Skew[n].\[Omega](n.\[Omega]d); (*feedforward term*)
	(*\[Tau]cl = \[Tau]ff -kp en-kd OP[n].(\[Omega]-\[Omega]d);*)
	\[Tau]cl = \[Tau]ff -kp en-kd #/Sqrt[\[Sigma]\[Sigma]^2+#.#]&[OP[n].(\[Omega]-\[Omega]d)];
	...
	V\[Theta] = kp(1-n.nd)+1/2 (#.#)&[e\[Omega]];
	W\[Theta] = - kd (#.#/Sqrt[\[Sigma]\[Sigma]^2+#.#])&[e\[Omega]];
	(*there are lypaunov functions for which Vdot is "negative definite" but we leave that for later work*)
